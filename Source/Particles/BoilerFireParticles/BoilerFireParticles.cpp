...
const std::string BoilerFireParticles::type = "boiler_fire";
const std::string BoilerFireParticles::textureName = "pirolisis.dds";


BoilerFireParticles::BoilerFireParticles()
: ParticleEmitter(50)
, radius(0.0f)
{
	timeout.init(0.33f);
...
void BoilerFireParticles::update( float deltaTime )
	{
...
void BoilerFireParticles::generate()
{
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		y = Global::obtainRandom(0.0f, 4.0f);
		x = Global::obtainRandom(-y * 0.15f, y * 0.15f);
		z = Global::obtainRandom(-y * 0.15f, y * 0.15f);
