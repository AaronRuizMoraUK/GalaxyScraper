...
RigidAnimationResource::~RigidAnimationResource()
{
	OutputDebugString("Destroying Rigid Animation Resource...\n");
	clear();
}
...
void RigidAnimationResource::getPosition( float time, D3DXVECTOR3 &pPosition ) const
{
	...
	float clampedLambda = Global::clamp(lambda, 0.0f, 1.0f);
	...
	if( frameLambda )
		Global::lerp(pPosition, positions[frameIndex], positions[frameIndex+1], 1.0f-frameLambda);
	else
		pPosition = positions[ frameIndex ];
#endif        
}
...
{        
	...
	float frameLambda = frameTime - frameIndex;
	if( frameLambda )
		Global::lerp(pRotation, rotations[frameIndex], rotations[frameIndex+1], 1.0f-frameLambda);
