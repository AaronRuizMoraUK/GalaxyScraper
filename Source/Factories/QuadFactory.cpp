...
QuadFactory * QuadFactory::getQuadFactory() {
	if(quadFactory == NULL)
...
IMesh * QuadFactory::createQuad(float radiusSideX, float radiusSideY, int numQuadVertexPerAxis) {
	...
	key.push_back(numQuadVertexPerAxis);
	...
	if(it != quadMeshMap.end())
		...
		IMesh * mesh = createQuadMesh(radiusSideX, radiusSideY, numQuadVertexPerAxis);
		quadMeshMap[key]=mesh;
		return mesh;
	}
}

IMesh * QuadFactory::createQuadMesh(float radiusSideX, float radiusSideY, unsigned int numQuadVertexPerAxis)
{
